...
public class Client implements MessageUpcall {
    ...
    public void upcall(ReadMessage message) throws IOException, ClassNotFoundException {
        ...
        if(bytes[bytes.length-1] == END){
            ...
            ida.ipl.Board newboard = new ida.ipl.Board(bytes);
     ...
     */
    private synchronized void sendBoard(ida.ipl.Board[] boards, int flag) throws IOException {
    ...
    private int solutions(ida.ipl.Board board, BoardCache cache) throws IOException {
        ...
        count++;
        if (board.distance() == 0) {
            ida.ipl.Board[] result = new ida.ipl.Board[1];
            result[0] = board;
